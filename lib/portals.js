...
const ROLE_ASSETS_FACILITIES = 'assets_facilities';
...
const DEPARTMENTS = [
  ...
  { key: PORTAL_FO, label: 'Finance Office' },
];

const ROLES_BY_DEPARTMENT = {
  [PORTAL_SERVICE]: [
    { key: ROLE_STM, label: 'STM' },
    { key: ROLE_SERVICE_ADVISOR, label: 'SA' },
    { key: ROLE_SENIOR_SERVICE_RECEPTIONIST, label: 'SSR' },
    { key: ROLE_SERVICE_RECEPTIONIST, label: 'SR' },
    { key: ROLE_TECHNICIAN, label: 'Tech' },
  ],
  [PORTAL_PARTS]: [
    { key: ROLE_PARTS_MANAGER, label: 'PM' },
    { key: ROLE_PARTS_CLERK, label: 'Clerk' },
  ],
  [PORTAL_STORES]: [
...
const LOCATIONS_BY_DEPARTMENT = {
  ...
  [PORTAL_GM]: HR_LOCATIONS.slice(),
  [PORTAL_FO]: HR_LOCATIONS.slice(),
};

const JOB_CODES_BY_ROLE = {
  [ROLE_GENERAL_MANAGER]: ['GENERAL MANAGER', 'GM', 'MANAGING DIRECTOR', 'PRESIDENT'],
  ...
  [ROLE_TECHNICIAN]: ['TECHNICIAN', 'MECHANIC', 'SENIOR MECHANIC', 'ALIGNER', 'CARWASHER', 'TOOLKEEPER', 'MECHANIC TRAINEE'],
  ...
  [ROLE_ASSETS_FACILITIES]: ['ASSETS FACILITIES', 'ASSETS & FACILITIES', 'A&F', 'FACILITIES', 'FTE'],
  [ROLE_ACCOUNTING]: ['ACCOUNTING', 'ACCOUNTANT', 'BOOKKEEPER'],
  [ROLE_FINANCE_MANAGER]: ['FINANCE MANAGER', 'FM', 'ACCOUNTING'],
  [ROLE_ADMIN]: ['ADMIN', 'ADMINISTRATOR', 'FINANCE OFFICE ADMIN'],
};
...
const ROLE_GRANTS = {
  ...
  [ROLE_ADMIN]: {
    ...
    [PORTAL_FO]: g(GRANT.access, GRANT.view, GRANT.edit, GRANT.modify_level_of_use, GRANT.manage_users),
  },
  [ROLE_FINANCE_MANAGER]: {
    ...
    [PORTAL_FO]: g(GRANT.access, GRANT.view, GRANT.view_financials, GRANT.print, GRANT.export),
  },
  [ROLE_ACCOUNTING]: {
    [PORTAL_SERVICE]: g(GRANT.limited_view, GRANT.view_financials),
    [PORTAL_PARTS]: g(GRANT.limited_view, GRANT.view_financials),
    ...
    [PORTAL_HR]: g(GRANT.limited_view, GRANT.view_payroll),
    [PORTAL_GM]: g(GRANT.access, GRANT.view, GRANT.view_financials, GRANT.print, GRANT.export),
    [PORTAL_FO]: g(GRANT.access, GRANT.view, GRANT.view_financials, GRANT.print, GRANT.export, GRANT.filloutform),
  },
  [ROLE_ASSETS_FACILITIES]: {
    [PORTAL_SERVICE]: g(GRANT.access, GRANT.view, GRANT.filloutform, GRANT.print, GRANT.request),
    [PORTAL_PARTS]: g(GRANT.limited_view),
    [PORTAL_STORES]: g(GRANT.limited_view),
    [PORTAL_HR]: [],
    [PORTAL_GM]: g(GRANT.access, GRANT.view),
    [PORTAL_FO]: g(GRANT.access, GRANT.view, GRANT.filloutform, GRANT.print, GRANT.request),
  },
};
...
const HOME_BY_ROLE = {
  ...
  [ROLE_ACCOUNTING]: '/finance',
  [ROLE_ASSETS_FACILITIES]: '/gm/fte',
...
const ROLE_SHORT_LABELS = {
  ...
  [ROLE_ACCOUNTING]: 'ACCT',
  [ROLE_ASSETS_FACILITIES]: 'A&F',
...
const PATH_PORTALS = [
  ...
  { prefix: '/admin', portal: PORTAL_FO },
  { prefix: '/finance', portal: PORTAL_FO },
  { prefix: '/api/finance', portal: PORTAL_FO },
...
function normalizeRole(value) {
  ...
  if (raw === 'acct' || raw === 'accounting' || raw === 'accountant') return ROLE_ACCOUNTING;
  if (
    raw === 'a&f'
    || raw === 'af'
    || raw === 'assets_facilities'
    || raw === 'assets & facilities'
    || raw === 'assets and facilities'
    || raw === 'assets'
    || raw === 'facilities'
  ) return ROLE_ASSETS_FACILITIES;
...
function departmentForRole(role) {
  ...
  if (normalized === ROLE_GENERAL_MANAGER) {
    return PORTAL_GM;
  }
  if (
    normalized === ROLE_ADMIN
    || normalized === ROLE_FINANCE_MANAGER
    || normalized === ROLE_ACCOUNTING
    || normalized === ROLE_ASSETS_FACILITIES
  ) {
    return PORTAL_FO;
  }
...
function employeeMatchesPortalRole(employee, role) {
  ...
  const jobCode = normalizeJobText(employee && employee.job_code);
  const jobTitle = normalizeJobText(employee && employee.job_title);
  if (jobCode && codes.includes(jobCode)) return true;
  if (jobTitle && codes.includes(jobTitle) && !jobCode) return true;
  if (jobCode) return false;
  const text = [jobCode, jobTitle].filter(Boolean).join(' ');
  if (!text) return false;
  return codes.some((token) => {
...
function canonicalizeLocation(department, value) {
  ...
  if (dept === PORTAL_SERVICE || dept === PORTAL_HR || dept === PORTAL_PARTS || dept === PORTAL_FO || dept === PORTAL_GM) {
...
function accountRoleMatches(accountRole, selectedRole) {
  ...
  if (right === ROLE_ACCOUNTING && (left === ROLE_FINANCE_MANAGER || left === 'fm')) return true;
  if (right === ROLE_FINANCE_MANAGER && left === ROLE_ACCOUNTING) return true;
  return false;
}
...
function accessiblePortals(role) {
  return [PORTAL_SERVICE, PORTAL_PARTS, PORTAL_STORES, PORTAL_HR, PORTAL_FO]
...
function portalHome(portal) {
  ...
  if (portal === PORTAL_FO) return '/admin';
  return '/gm';
}
