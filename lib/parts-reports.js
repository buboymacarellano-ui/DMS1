...
function inventoryReportRows(data) {
  return inventory.allAuditRows(data).filter((row) => {
    if (isPartsActivityLog(row)) return false;
    if (normalizePartsTransactionType(row.transaction_type || row.type) !== TYPE_SOLD) return true;
    return !sameLocation(locationOf(row), WAREHOUSE_1);
  });
}

function isWarehouse1Location(value) {
  return sameLocation(value, WAREHOUSE_1);
}

function warehouseReportRows(rows, warehouseQuery) {
  const filtered = rows.filter((row) => sameLocation(locationOf(row), warehouseQuery));
  if (!isWarehouse1Location(warehouseQuery)) return filtered;
  // Warehouse 1 does not sell. Stock leaves only by transfer, never as Sold.
  return filtered.filter((row) => normalizePartsTransactionType(row.transaction_type || row.type) !== TYPE_SOLD);
}
...
function buildGeneratedReport(data, query) {
  ...
  const allRows = inventoryReportRows(data);
  ...
  if (type === 'lifecycle') {
    ...
    return {
      ...
      tables: [{
        ...
        rows: history.history.filter((row) => !isPartsActivityLog(row)).map((row) => mapTxRow(data, row)),
      }],
    };
  }
  ...
  if (type === 'warehouse') {
    ...
    const filtered = warehouseReportRows(allRows, warehouse);
    return {
      ok: true,
      type,
      title,
      generatedAt,
      subtitle: isWarehouse1Location(warehouse)
        ? `${warehouse} · restock and transfers only (Warehouse 1 does not sell)`
        : `Warehouse / location: ${query.warehouse}`,
      tables: [{
