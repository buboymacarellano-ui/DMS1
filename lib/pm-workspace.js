...
const { fulfillmentLabel } = require('./parts-transfer-receive');
...
function buildPmApprovals(data) {
  ...
  const approvedTransactions = (data.parts_inventory || [])
    .filter((row) => row && row.approved_at)
    .sort((a, b) => {
    ...
    .slice(0, 80)
    .map((row) => Object.assign({}, row, {
      fulfillment_label: fulfillmentLabel(row),
    }));
