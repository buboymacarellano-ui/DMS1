...
function isWarehouse1SoldRow(row) {
  if (!row) return false;
  if (normalizePartsTransactionType(row.transaction_type || row.type) !== TYPE_SOLD) return false;
  return sameLocation(partLocation(row), WAREHOUSE_1);
}

function filterRowsByLocation(rows, location) {
  if (!location) return Array.isArray(rows) ? rows : [];
  const scoped = (rows || []).filter((row) => belongsToLocation(row, location));
  if (!sameLocation(location, WAREHOUSE_1)) return scoped;
  return scoped.filter((row) => !isWarehouse1SoldRow(row));
}
