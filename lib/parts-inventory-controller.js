...
function transactionType(row) {
  return normalizePartsTransactionType(row && (row.type || row.transaction_type));
}
...
function getDashboardLogs(data, query) {
  ...
  ensureCollections(data);
  // Conceptual filter for the active dashboard view.
  // Source is the immutable transactions trail (plus inventory rows not yet copied).
  const transactions = allAuditRows(data);
  const activeDashboardLogs = transactions.filter((t) => {
    if (transactionType(t) === TYPE_RESTOCK && isOlderThanThreeWorkingDays(transactionDate(t), now)) {
      return false; // Hides it from the daily dashboard view
    }
    return true; // Keeps everything else, including audit data
  });
