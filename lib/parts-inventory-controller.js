...
function rememberTransaction(data, record) {
  ...
  try {
    require('./parts-stock-alerts').maybeReconcileFromRecord(data, record);
  } catch (_) { /* stock alerts are optional for catalog rebuilds */ }
  return record;
}
...
function getDashboardLogs(data, query) {
  ...
  const activeDashboardLogs = transactions.filter((t) => {
    ...
    if (type === TYPE_SOLD && sameLocation(t.present_location || t.branch, WAREHOUSE_1)) return false;
    if (type === PARTS_REQUEST_TYPE) {
      const status = String(t.request_status || 'pending').trim().toLowerCase();
      if (status === 'rejected') return false;
      if ((status === 'approved' || status === 'forwarded') && !t.approved_at) return false;
    }
